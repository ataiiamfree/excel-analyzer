...
- **单次调用 token 有硬上限**：任何一次 LLM 调用的输入不超过 4K tokens
...
def _extract_summary(self, stdout: str, max_chars: int = 200) -> str:
    ...
    return '\n'.join(summary_lines) if summary_lines else stdout[:max_chars]
```
...
class PromptAssembler:
    """为每一步组装独立的 Prompt，保证不超 4K tokens"""

    def assemble(self, context: TaskContext, current_step: Step) -> str:
        sections = []

        # 1. 系统指令（固定）
        sections.append(self._load_system_prompt(current_step.type))
        ...
        # 安全兜底：超预算则压缩摘要区
        while self._count_tokens(prompt) > 4000:
...
**总计约 2000 tokens。** 包含第 3 步所需全部信息，但无前两步原始输出/代码/数据。
...
class ExcelPreprocessor:
    ...
    def process(self, file_path: str, manifest: dict) -> PreprocessResult:
        ...
        for table_candidate in manifest["tables"]:
            ws = wb[table_candidate["sheet_name"]]
...
class Orchestrator:
    ...
    async def run(self, user_query: str, excel_path: str) -> TaskResult:
        ...
        # ── Phase 0: Workbook 扫描 + Excel 预处理（规则优先，低置信度才调 LLM）──
        ...
        # ── Phase 2: 粗略规划（Sketch，1 次 LLM 调用）──
        context = TaskContext(
...
class Profiler:
    ...
    def _group_similar_columns(self, columns):
        ...
        for pattern, cols in groups.items():
            if len(cols) >= 3:
                nums = [re.search(r'\d+', c["name"]).group() for c in cols]
                grouped.append({
...
Token 预算根据所用模型的上下文窗口配置。提供两个预设方案：

```
                    standard（32K 窗口）    generous（128K+ 窗口）
                    本地小模型               内网大上下文模型
  ──────────────────────────────────────────────────────────────
  单次调用输入上限    4K tokens              16K tokens
  数据画像预算        800 tokens             3K tokens
  步骤摘要(每条)      300 字                 800 字
  步骤摘要(总量)      1K tokens              4K tokens
  关键发现            10 条                  20 条
  列名分组阈值        50 列以上才分组         100 列以上才分组
  报告每章上下文      ~4K                    ~12K
  ──────────────────────────────────────────────────────────────
  使用 generous 方案时，profile 更完整、摘要更详细，
  LLM 能获得更多上下文 → 代码生成和报告质量更高。
```
