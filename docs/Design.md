...
│   │   ├── planner.py              # 规划器
│   │   ├── reporter.py             # 报告生成器
│   │   └── next_actions.py         # 基于分析摘要生成下一步行动建议
│   │
│   ├── context/
│   │   ├── __init__.py
...
│   └── src/artifacts/              # 图表、表格、文件预览与产物去重工具
│
...
- 侧边栏底部用户卡片只展示头像、用户名和设置入口，不展示 `Local · API` 等状态标签。
- 首页只保留左侧会话栏和右侧居中的上传主区，不放装饰性原型卡片；上传表单宽度按主区居中约束控制。
- 产物面板中的预览图标必须切换到可预览产物并定位卡片；表格和图片类产物的放大按钮打开大尺寸预览层；代码 tab 只展示源码类产物，不展示 `report.md` 等报告文件；右侧面板按 `kind + name` 对同名产物去重，默认保留最新版本。
- 首页上传表单 textarea 支持 `Enter` 发送、`Shift+Enter` 换行，并且中文输入法 composing 状态不能触发提交。
- 右侧产物面板只能由顶部栏右侧的单个 toggle 控制：面板打开时显示隐藏图标，面板隐藏时显示打开图标；面板标题区不再放单独的隐藏按钮。
- 对话框上方的下一步行动建议由后端在分析完成后通过 LLM 基于报告摘要、步骤反馈和产物清单生成，写入 assistant payload 的 `next_actions`；前端禁止渲染写死建议，且只在对话滚动停留在最新消息底部时展示。
...
type ServerEvent =
  ...
  | { type: "run.complete"; seq: number; message_id: string; report: string; result?: AssistantMessagePayload }
...
- `app/agent/runtime.py` 提供 `AgentRuntimeAdapter`、`PiSidecarRuntimeAdapter`、`PiRpcTransport`、`FallbackAgentRuntimeAdapter` 和 `OrchestratorRuntimeAdapter`。默认 `AGENT_RUNTIME=pi`，API 层优先通过 Pi RPC sidecar 调用 agent；`AGENT_RUNTIME=orchestrator` 可强制使用旧 Orchestrator，`AGENT_RUNTIME_FALLBACK=true` 会在 Pi 不可用时自动回退。
...
这部分实现的边界是：生产环境要真正走 Pi primary runtime，需要安装 Pi CLI 或配置兼容的 `PI_COMMAND`，并准备可用模型凭证；Milestone 7 中的非 Excel 数据源和更多办公输出形态尚未展开。
