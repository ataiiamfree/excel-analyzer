...
Phase 8: API 与前端                   ← 能给人用、能取消/重跑/预览（2天）
...
## 九、Phase 8: API 与前端（能给人用）

### 目标
提供 Web 界面，可以上传 Excel、输入问题、查看结果。

### Step 8.1: FastAPI Server
文件：`app/server.py`

接口：
```
POST /api/analyze        上传文件 + 问题 → 返回 task_id
GET  /api/task/{id}/status   查询任务状态和进度
GET  /api/task/{id}/result   获取最终结果（报告 + 文件列表）
GET  /api/task/{id}/artifacts 获取 artifact_manifest
GET  /api/task/{id}/files/{name}  下载生成的文件
POST /api/task/{id}/cancel    取消本地任务
POST /api/task/{id}/rerun-step/{step_id}  重跑某一步
```

实现要点：
- 文件上传用 `UploadFile`
- 单用户版本可用 `asyncio.create_task` 或本地 `TaskRunner`
...
- 结果展示区：
  ...
  - 数据表格预览（前 20 行）
  - 文件下载按钮
  - workbook/table 选择信息和预处理 warnings
  - 任务失败时展示最后错误、失败步骤、可重跑按钮

### Step 8.3: SSE 进度推送（可选增强）
如果轮询体验不好，可以用 Server-Sent Events 实时推送进度：
```python
