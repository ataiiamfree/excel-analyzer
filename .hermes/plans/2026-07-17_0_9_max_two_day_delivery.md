...
## Phase 2：视频演示与 PPT 就绪度优化（待用户确认后执行）
...
3. 标准答案：杭州西湖店 753.3、北京朝阳店 739.7、上海浦东店 664.9、广州天河店 664.8、成都高新店 566.7。
...
- [ ] 固定演示问题连续两次得出正确主结论和指定产物。
- [ ] 1440×900 与 1920×1080 关键画面无裁切、遮挡或浮点尾差。
- [ ] 默认画面无个人信息、绝对路径、token 数、stdout 或英文调试标签。
- [ ] 90–120 秒视频脚本与 3 分钟现场脚本均完成一次 dry run。
- [ ] 6 张核心截图可直接放入 PPT，且来自真实运行。
- [ ] PPT 的测试数字、benchmark、版本号和已知限制与 release note 一致。
- [ ] 已准备网络/模型异常时的诚实备用会话与切换话术。
- [ ] 新增演示优化未破坏上传、分析、追问、下载和刷新恢复。
