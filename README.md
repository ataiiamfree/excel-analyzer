...
上传 Excel 后可输入分析需求、查看分步进度和报告/图表；追问时无需重新上传。首页上传区支持点击选择或直接拖拽文件到区域内，选错文件可点右侧的 × 一键移除重选。当前支持 `.xlsx`、`.xlsm`，默认文件上限为 100 MB。

分析完成后，报告下方的操作栏可复制文本、导出 Markdown，或点「PDF」在打印视图中另存为 PDF（含结论、正文、图表与数据产物清单）。
